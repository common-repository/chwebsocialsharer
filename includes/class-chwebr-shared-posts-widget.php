 ...
 *
 */
class chwebr_mostshared_posts_widget extends WP_Widget {

    function __construct() {
        parent::__construct( false, $name = __( 'ChwebSocialShare - Most Shared Posts', 'chwebr' ) );
    }
    ...
    public function form( $instance ) {
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Widget Title', 'chwebr' ); ?></label>
            <input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo $title; ?>" />
        </p>

        <p>
            <label for="<?php echo $this->get_field_id( 'count' ); ?>"><?php _e( 'How many posts to display?', 'chwebr' ); ?></label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'excerpt_length' ); ?>"><?php _e( 'How many characters for the excerpt? Use 0 for not showing!', 'chwebr' ); ?></label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'title_length' ); ?>"><?php _e( 'How many characters for the title?', 'chwebr' ); ?></label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'image_size' ); ?>"><?php _e( 'Width of the image in px? Use 0 for not showing a thumbnail', 'chwebr' ); ?></label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'showShares' ); ?>"><?php _e( 'Show Shares? Say "No" when using fake shares!', 'chwebr' ); ?></label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'countLabel' ); ?>"><?php _e( 'Share Count Label', 'chwebr' ); ?></label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'period' ); ?>"><?php _e( 'Time period and age of posts', 'chwebr' ); ?></label>
    ...
    public function widget( $args, $instance ) {
        ...
        echo '<!-- ChwebSocialShare Most Popular Widget //-->';
        echo $before_widget;
        // Display the widget
        ...
        //var_dump($wpq);
        if( $wpq->have_posts() ) :
            echo '<ul class="chwebr-share-widget">';
            while ( $wpq->have_posts() ):
                ...
                if (!empty($image_url)){
                    ...
                    $image = !empty($image_size) ? '<div class="chwebr-widget-img" style="' . $css . '"><a class="chwebr-widget-link" href="' . get_the_permalink() . '" style="display:block;width:'.$image_size.'px;height:' . $image_size.'px;">&nbsp</a></div>' : '';
  
                } else {
                    $css = 'display:block;width:'.$image_size.'px;height:' . $image_size.'px;';
                    $image = !empty($image_size) ? '<div class="chwebr-widget-img" style="' . $css . '"><a class="chwebr-widget-link" href="' . get_the_permalink() . '">&nbsp</a></div>' : '';
                }
                               
   
                $title_result = '<div class="chwebr-widget-post-title"><a class="chwebr-widget-link" href="' . get_the_permalink() . '">' . $this->limit_title(get_the_title(), $title_length) . '</a></div>';

                $excerpt =  !empty($excerpt_length) ? '<div class="chwebr-excerpt">' . $this->limit_excerpt(get_the_excerpt($postID), $excerpt_length) . '</div>' : '';

                
                if( $showShares === 'true' ):
                    $shares = get_post_meta( $postID, 'chwebr_shares', true ) + getFakecount();
                    echo '<li>' . $image .  $title_result . $excerpt . ' <span class="chwebicon-share">' . roundshares( $shares ) . ' ' . $countLabel . '</span></li>';
                else:
        ...
        wp_reset_postdata();
        echo $after_widget;
        echo '<!-- ChwebSocialShare Most Popular Widget End //-->';
    }
    ...
    public function get_qry_from_cache( $args ) {
        ...
        if (CHWEBR_DEBUG){
            delete_transient('chwebwidget_' . md5( json_encode( $args ) ));
        }
        
        if( false === ( $qry = get_transient( 'chwebwidget_' . md5( json_encode( $args ) ) ) ) ) {
            $wpq = new WP_Query( $args );
            set_transient( 'chwebwidget_' . md5( json_encode( $args ) ), $wpq, $expiration );
                        //wp_die( var_dump($wpq));  
            return $wpq; 
        } else {
                                //wp_die( var_dump($qry) );
...
/* Register Widget */
function chwebr_register_widget() {
    register_widget( 'chwebr_mostshared_posts_widget' );
}
add_action( 'widgets_init', 'chwebr_register_widget', 1 );
