 ...
 */
abstract class CHWEBR_RWMB_Field
{
	/**
	 * Add actions
	 *
	...
	static function show( $field, $saved )
	{
		...
		$field_class = CHWEBR_RW_Meta_Box::get_class_name( $field );
		$meta        = call_user_func( array( $field_class, 'meta' ), $post_id, $saved, $field );
		$meta        = CHWEBR_RWMB_Core::filter( 'field_meta', $meta, $field, $saved );

		$begin = call_user_func( array( $field_class, 'begin_html' ), $meta, $field );
		$begin = CHWEBR_RWMB_Core::filter( 'begin_html', $begin, $field, $meta );
		...
		if ( $field['clone'] )
		{
			...
			foreach ( $meta as $index => $sub_meta )
			{
				...
				// Wrap field HTML in a div with class="chwebr-rwmb-clone" if needed
				$class     = "chwebr-rwmb-clone chwebr-rwmb-{$field['type']}-clone";
				$sort_icon = '';
				if ( $field['sort_clone'] )
				{
					$class .= ' chwebr-rwmb-sort-clone';
					$sort_icon = "<a href='javascript:;' class='chwebr-rwmb-clone-icon'></a>";
				}
				$input_html = "<div class='$class'>" . $sort_icon;

				// Call separated methods for displaying each type of field
				$input_html .= call_user_func( array( $field_class, 'html' ), $sub_meta, $sub_field );
				$input_html = CHWEBR_RWMB_Core::filter( 'html', $input_html, $sub_field, $sub_meta );
		...
		else
		{
			...
			$field_html = CHWEBR_RWMB_Core::filter( 'html', $field_html, $field, $meta );
		}

		$end = call_user_func( array( $field_class, 'end_html' ), $meta, $field );
		$end = CHWEBR_RWMB_Core::filter( 'end_html', $end, $field, $meta );

		$html = CHWEBR_RWMB_Core::filter( 'wrapper_html', "$begin$field_html$end", $field, $meta );

		// Display label and input in DIV and allow user-defined classes to be appended
		$classes = "chwebr-rwmb-field chwebr-rwmb-{$field['type']}-wrapper " . $field['class'] ;
		...
		$outer_html = CHWEBR_RWMB_Core::filter( 'outer_html', $outer_html, $field, $meta );
	...
	static function begin_html( $meta, $field )
	{
		...
		{
			$field_label = sprintf(
				'<div class="chwebr-rwmb-label"><label for="%s">%s</label></div>',
				$field['id'],
				$field['name']
			);
		}
	...
	{
		$button = $field['clone'] ? call_user_func( array( CHWEBR_RW_Meta_Box::get_class_name( $field ), 'add_clone_button' ), $field ) : '';
	...
	static function add_clone_button( $field )
	{
		...
		return "<a href='#' class='chwebr-rwmb-button button-primary add-clone'>$text</a>";
	}
	...
	static function remove_clone_button( $field )
	{
		...
		return "<a href='#' class='chwebr-rwmb-button remove-clone'>$text</a>";
	}
	...
	static function meta( $post_id, $saved, $field )
	{
		...
		// Escape attributes
		$meta = call_user_func( array( CHWEBR_RW_Meta_Box::get_class_name( $field ), 'esc_meta' ), $meta );
	...
	{
		$value  = call_user_func( array( CHWEBR_RW_Meta_Box::get_class_name( $field ), 'get_value' ), $field, $args, $post_id );
		$output = $value;
		if ( is_array( $value ) )
		{
			$output = '<ul>';
