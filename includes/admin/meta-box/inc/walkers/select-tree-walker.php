 ...
 * @uses CHWEBR_RWMB_Select_Walker
 */
class CHWEBR_RWMB_Select_Tree_Walker
{
	/**
	 * Field data.
	 * @var string
	...
	function display_level( $options, $parent_id = 0, $active = false )
	{
		...
		$field       = $this->field;
		$meta        = $this->meta;
		$walker      = new CHWEBR_RWMB_Select_Walker( $this->db_fields, $this->field, $this->meta );
		...
		$output   = sprintf(
			'<div class="chwebr-rwmb-select-tree %s" data-parent-id="%s"><select %s>',
			$active ? '' : 'hidden',
			$parent_id,
			CHWEBR_RWMB_Field::render_attributes( $attributes )
		);
