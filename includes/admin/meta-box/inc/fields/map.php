 ...
 * Map field class.
 */
class CHWEBR_RWMB_Map_Field extends CHWEBR_RWMB_Field
{
	/**
	...
	static function admin_enqueue_scripts()
	{
		...
		wp_enqueue_style( 'chwebr-rwmb-map', CHWEBR_RWMB_CSS_URL . 'map.css' );
		wp_enqueue_script( 'chwebr-rwmb-map', CHWEBR_RWMB_JS_URL . 'map.js', array( 'jquery-ui-autocomplete', 'google-maps' ), CHWEBR_RWMB_VER, true );
	}
	...
	{
		$html = '<div class="chwebr-rwmb-map-field">';

		$html .= sprintf(
			'<div class="chwebr-rwmb-map-canvas" data-default-loc="%s"></div>
			<input type="hidden" name="%s" class="chwebr-rwmb-map-coordinate" value="%s">',
			esc_attr( $field['std'] ),
		...
		{
			$html .= sprintf(
				'<button class="button chwebr-rwmb-map-goto-address-button" value="%s">%s</button>',
	...
	static function the_value( $field, $args = array(), $post_id = null )
	{
		...
		$google_maps_url = apply_filters( 'rwmb_google_maps_url', 'https://maps.google.com/maps/api/js?sensor=false' );
		wp_register_script( 'google-maps', esc_url_raw( $google_maps_url ), array(), '', true );
		wp_enqueue_script( 'chwebr-rwmb-map-frontend', CHWEBR_RWMB_JS_URL . 'map-frontend.js', array( 'google-maps' ), '', true );
		...
		$output = sprintf(
			'<div class="chwebr-rwmb-map-canvas" data-map_options="%s" style="width:%s;height:%s"></div>',
