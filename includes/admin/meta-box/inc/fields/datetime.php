 ...
 */
class CHWEBR_RWMB_Datetime_Field extends CHWEBR_RWMB_Text_Field
{
	/**
	...
	{
		$url = CHWEBR_RWMB_CSS_URL . 'jqueryui';
		wp_register_style( 'jquery-ui-core', "{$url}/jquery.ui.core.css", array(), '1.8.17' );
		wp_register_style( 'jquery-ui-theme', "{$url}/jquery.ui.theme.css", array(), '1.8.17' );
		wp_register_style( 'wp-datepicker', CHWEBR_RWMB_CSS_URL . 'datepicker.css', array( 'jquery-ui-core', 'jquery-ui-theme' ), '1.8.17' );
		...
		$url = CHWEBR_RWMB_JS_URL . 'jqueryui';
		...
		{
			if ( file_exists( CHWEBR_RWMB_DIR . 'js/' . $date_path ) )
			{
				wp_register_script( 'jquery-ui-datepicker-i18n', CHWEBR_RWMB_JS_URL . $date_path, array( 'jquery-ui-datepicker' ), '1.8.17', true );
				$deps[] = 'jquery-ui-datepicker-i18n';
				break;
			}
		}

		wp_register_script( 'chwebr-rwmb-datetime', CHWEBR_RWMB_JS_URL . 'datetime.js', $deps, CHWEBR_RWMB_VER, true );
		wp_register_script( 'chwebr-rwmb-date', CHWEBR_RWMB_JS_URL . 'date.js', $deps, CHWEBR_RWMB_VER, true );
		wp_register_script( 'chwebr-rwmb-time', CHWEBR_RWMB_JS_URL . 'time.js', array( 'jquery-ui-timepicker-i18n' ), CHWEBR_RWMB_VER, true );
		wp_localize_script( 'chwebr-rwmb-datetime', 'CHWEBR_RWMB_Datetimepicker', array(
			'locale'      => $locale,
			'localeShort' => $locale_short,
		) );
		wp_localize_script( 'chwebr-rwmb-time', 'CHWEBR_RWMB_Timepicker', array(
			'locale'      => $locale,
			'localeShort' => $locale_short,
		) );
	}
	...
	public static function admin_enqueue_scripts()
	{
		...
		wp_enqueue_script( 'chwebr-rwmb-datetime' );
	}
	...
	public static function html( $meta, $field )
	{
		...
		if ( $field['timestamp'] )
		{
			...
			$output .= sprintf(
				'<input type="hidden" name="%s" class="chwebr-rwmb-datetime-timestamp" value="%s">',
		...
		if ( $field['inline'] )
		{
			$output .= '<div class="chwebr-rwmb-datetime-inline"></div>';
		}
	...
	public static function meta( $post_id, $saved, $field )
	{
		...
		$method = array( CHWEBR_RW_Meta_Box::get_class_name( $field ), 'translate_format' );
		if ( is_array( $meta ) )
		{
	...
	public static function normalize( $field )
	{
		...
		$field = CHWEBR_RWMB_Text_Field::normalize( $field );
