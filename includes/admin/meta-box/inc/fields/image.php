 ...
 */
class CHWEBR_RWMB_Image_Field extends CHWEBR_RWMB_File_Field
{
	/**
	...
	static function admin_enqueue_scripts()
	{
		...
		wp_enqueue_style( 'chwebr-rwmb-image', CHWEBR_RWMB_CSS_URL . 'image.css', array(), CHWEBR_RWMB_VER );
		wp_enqueue_script( 'chwebr-rwmb-image', CHWEBR_RWMB_JS_URL . 'image.js', array( 'jquery-ui-sortable' ), CHWEBR_RWMB_VER, true );
	}
	...
	static function wp_ajax_reorder_images()
	{
		...
		check_ajax_referer( "chwebr-rwmb-reorder-images_{$field_id}" );
	...
	{
		$reorder_nonce = wp_create_nonce( "chwebr-rwmb-reorder-images_{$field['id']}" );
		$delete_nonce  = wp_create_nonce( "chwebr-rwmb-delete-file_{$field['id']}" );
		$classes       = array( 'chwebr-rwmb-images', 'chwebr-rwmb-uploaded' );
		if ( count( $images ) <= 0 )
			$classes[] = 'hidden';
	...
	static function img_html( $image )
	{
		...
		$item        = '
			<li id="item_%s">
				<img src="%s" />
				<div class="chwebr-rwmb-image-bar">
					<a title="%s" class="chwebr-rwmb-edit-file" href="%s" target="_blank">%s</a> |
					<a title="%s" class="chwebr-rwmb-delete-file" href="#" data-attachment_id="%s">&times;</a>
				</div>
			</li>
		';
