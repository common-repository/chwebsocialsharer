 ...
 */
class CHWEBR_RWMB_File_Field extends CHWEBR_RWMB_Field
{
	/**
	...
	{
		wp_enqueue_style( 'chwebr-rwmb-file', CHWEBR_RWMB_CSS_URL . 'file.css', array(), CHWEBR_RWMB_VER );
		wp_enqueue_script( 'chwebr-rwmb-file', CHWEBR_RWMB_JS_URL . 'file.js', array( 'jquery' ), CHWEBR_RWMB_VER, true );
		wp_localize_script( 'chwebr-rwmb-file', 'rwmbFile', array(
	...
	static function wp_ajax_reorder_files()
	{
		...
		check_ajax_referer( "chwebr-rwmb-reorder-files_{$field_id}" );
	...
	static function wp_ajax_delete_file()
	{
		...
		check_ajax_referer( "chwebr-rwmb-delete-file_{$field_id}" );
	...
	static function html( $meta, $field )
	{
		...
		$html .= sprintf(
			'<div class="%s">
				...
				<a class="chwebr-rwmb-add-file" href="#"><strong>%s</strong></a>
			</div>',
	...
	{
		$reorder_nonce = wp_create_nonce( "chwebr-rwmb-reorder-files_{$field['id']}" );
		$delete_nonce  = wp_create_nonce( "chwebr-rwmb-delete-file_{$field['id']}" );

		$classes = array( 'chwebr-rwmb-file', 'chwebr-rwmb-uploaded' );
		if ( count( $files ) <= 0 )
			$classes[] = 'hidden';
	...
	static function file_html( $attachment_id )
	{
		...
		$item        = '
		<li id="item_%s">
			<div class="chwebr-rwmb-icon">%s</div>
			<div class="chwebr-rwmb-info">
				<a href="%s" target="_blank">%s</a>
				<p>%s</p>
				<a title="%s" href="%s" target="_blank">%s</a> |
				<a title="%s" class="chwebr-rwmb-delete-file" href="#" data-attachment_id="%s">%s</a>
			</div>
		</li>';
	...
	static function get_value( $field, $args = array(), $post_id = null )
	{
		...
		{
			if ( $file_info = call_user_func( array( CHWEBR_RW_Meta_Box::get_class_name( $field ), 'file_info' ), $file_id, $args ) )
			{
