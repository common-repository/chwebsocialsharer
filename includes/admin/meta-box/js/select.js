jQuery( function ( $ )
{
	...
	var select = window.rwmbSelect = {
		...
		selectAllNone: function ( $input )
		{
			...
			$input.on( 'click', '.chwebr-rwmb-select-all-none a', function ( e )
			{
				e.preventDefault();
		...
		{
			var $input = $el.closest( '.chwebr-rwmb-input' ),
				$clone = $input.find( '.chwebr-rwmb-clone' );
	...
	$( ':input.chwebr-rwmb-select' ).each( update );
	$( '.chwebr-rwmb-input' ).on( 'clone', ':input.chwebr-rwmb-select', update );
} );
