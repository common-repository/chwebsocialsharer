...
jQuery( function ( $ )
{
	...
	UploadButton = views.UploadButton = Backbone.View.extend( {
		className: 'chwebr-rwmb-upload-area',
		tagName  : 'div',
		template: wp.template( 'chwebr-rwmb-upload-area' ),
		render   : function ()
		{
		...
		initialize: function ( options )
		{
			...
			this.el.id = _.uniqueId( 'chwebr-rwmb-upload-area-');
			this.render();

			//Areas
			this.dropzone = this.el;
			this.browser  = this.$('.chwebr-rwmb-browse-button')[0];
	...
	function init()
	{
		new FileUploadField( { input: this, el: $( this ).siblings( 'div.chwebr-rwmb-media-view' ) } );
	}
	$( ':input.chwebr-rwmb-file_upload' ).each( init );
	$( '.chwebr-rwmb-input' )
		.on( 'clone', ':input.chwebr-rwmb-file_upload', init )
} );
