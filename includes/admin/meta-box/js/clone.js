...
jQuery( function ( $ )
{
	...
	var cloneIndex = {
		/**
		 * Set index for fields in a .chwebr-rwmb-clone
		 * @param $clone .chwebr-rwmb-clone element
		...
		set: function ( $clone, index )
		{
			...
			var $address = $clone.find( '.chwebr-rwmb-map-goto-address-button' );
			if ( $address.length )
			{
		 ...
		 * @param $container .chwebr-rwmb-input container
		 * @return integer
		 */
	...
	{
		var $last = $container.children( '.chwebr-rwmb-clone:last' ),
			$clone = $last.clone(),
	...
	function toggleAddButton( $container )
	{
		var $button = $container.find( '.add-clone' ),
			...
			numClone = $container.find( '.chwebr-rwmb-clone' ).length;
	...
	// Add clones
		.on( 'click', '.add-clone', function ( e )
		{
			...
			var $container = $( this ).closest( '.chwebr-rwmb-input' );
			clone( $container );
		...
		.on( 'click', '.remove-clone', function ( e )
		{
			...
			var $this = $( this ),
				$container = $this.closest( '.chwebr-rwmb-input' );

			// Remove clone only if there are 2 or more of them
			if ( $container.find( '.chwebr-rwmb-clone' ).length < 2 )
			{
				return;
			}
	...
	$( '.chwebr-rwmb-input' ).each( function ()
	{
		var $container = $( this );
		toggleRemoveButtons( $container );
		toggleAddButton( $container );
		...
		$container
			.data( 'next-index', $container.children( '.chwebr-rwmb-clone' ).length )
			.sortable( {
				handle     : '.chwebr-rwmb-clone-icon',
				placeholder: ' chwebr-rwmb-clone chwebr-rwmb-clone-placeholder',
				items      : '.chwebr-rwmb-clone',
