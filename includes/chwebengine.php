...
class chwebengine {
    ...
    public function getSharesFBTW() {
        ...
        $fb_mode = isset( $chwebr_options['facebook_count_mode'] ) ? $chwebr_options['facebook_count_mode'] : 'total';
        ...
        switch ( $fb_mode ) {
            case $fb_mode === 'likes':
                if( empty( $chwebr_options['fb_access_token_new'] ) ) {
                    if( isset( $chwebr_options['cumulate_http_https'] ) ) {
                    ...
                    if( isset( $chwebr_options['cumulate_http_https'] ) ) {
                        $RollingCurlX->addRequest( "https://graph.facebook.com/v2.7/?id=" . $this->http_scheme_url . '&access_token=' . sanitize_text_field( $chwebr_options['fb_access_token_new'] ), $post_data, array($this, 'getCount'), array('facebook_likes'), $headers );
                        $RollingCurlX->addRequest( "https://graph.facebook.com/v2.7/?id=" . $this->https_scheme_url . '&access_token=' . sanitize_text_field( $chwebr_options['fb_access_token_new'] ), $post_data, array($this, 'getCount'), array('facebook_likes'), $headers );
                    } else {
                        $RollingCurlX->addRequest( "https://graph.facebook.com/v2.7/?id=" . $this->url . '&access_token=' . sanitize_text_field( $chwebr_options['fb_access_token_new'] ), $post_data, array($this, 'getCount'), array('facebook_likes'), $headers );
                    }
                }
                break;
            case $fb_mode === 'total':
                if( empty( $chwebr_options['fb_access_token_new'] ) ) {
                    ...
                    if( isset( $chwebr_options['cumulate_http_https'] ) ) {
                        $RollingCurlX->addRequest( "https://graph.facebook.com/v2.7/?id=" . $this->http_scheme_url . '&access_token=' . sanitize_text_field( $chwebr_options['fb_access_token_new'] ), $post_data, array($this, 'getCount'), array('facebook_total'), $headers );
                        $RollingCurlX->addRequest( "https://graph.facebook.com/v2.7/?id=" . $this->https_scheme_url . '&access_token=' . sanitize_text_field( $chwebr_options['fb_access_token_new'] ), $post_data, array($this, 'getCount'), array('facebook_total'), $headers );
                    } else {
                        $RollingCurlX->addRequest( "https://graph.facebook.com/v2.7/?id=" . $this->url . '&access_token=' . sanitize_text_field( $chwebr_options['fb_access_token_new'] ), $post_data, array($this, 'getCount'), array('facebook_total'), $headers );
                    }
                }
                break;
            default:
                if( empty( $chwebr_options['fb_access_token_new'] ) ) {
                    ...
                    if( isset( $chwebr_options['cumulate_http_https'] ) ) {
                        $RollingCurlX->addRequest( "https://graph.facebook.com/v2.7/?id=" . $this->http_scheme_url . '&access_token=' . sanitize_text_field( $chwebr_options['fb_access_token_new'] ), $post_data, array($this, 'getCount'), array('facebook_shares'), $headers );
                        $RollingCurlX->addRequest( "https://graph.facebook.com/v2.7/?id=" . $this->https_scheme_url . '&access_token=' . sanitize_text_field( $chwebr_options['fb_access_token_new'] ), $post_data, array($this, 'getCount'), array('facebook_shares'), $headers );
                    } else {
                        $RollingCurlX->addRequest( "https://graph.facebook.com/v2.7/?id=" . $this->url . '&access_token=' . sanitize_text_field( $chwebr_options['fb_access_token_new'] ), $post_data, array($this, 'getCount'), array('facebook_shares'), $headers );
                    }
                }
        }
        if( isset( $chwebr_options['cumulate_http_https'] ) ) {
    ...
    function getCount( $data, $url, $request_info, $service, $time ) {
        ...
        if( $data ) {
            ...
            CHWEBR()->logger->info( 'ChwebEngine - URL: ' . $url . ' ' . $service[0] . ': ' . $count );
            chwebdebug()->info( 'ChwebEngine - URL: ' . $url . ' ' . $service[0] . ': ' . $count );
            //echo 'ChwebEngine - URL: ' . $url . ' ' . $service[0] . ': ' . $count . '<br>';
            $this->debug_notices[] = 'ChwebEngine - URL: ' . $url . ' ' . $service[0] . ': ' . $count . '<br>';
            $chwebr_error[] = 'ChwebEngine - URL: ' . $url . ' ' . $service[0] . ': ' . $count;
    ...
    public function setRateLimitTransient() {
        set_transient( 'chwebr_rate_limit', 'true', 60 * 60 );

        CHWEBR()->logger->info( 'Error: Facebook Rate Limit hit' );
